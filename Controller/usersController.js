...
//Создаем контролер регистрации пользователя 
...
exports.singin = (req, res ) => {
    db.query("SELECT `id`, `email`, `password` FROM `users` WHERE `email` = '"+req.body.email+"'",(error, rows, fields) =>{
        if(error){
            ...
            response.status(404, {message: `Пользователь с email - ${req.body.email} не найден. Пройдите регистрацию.`}, res)
        } else {
            const row = JSON.parse(JSON.stringify(rows))
            row.map(rw => {
               const password = bcrypt.compareSync(req.body.password, rw.password)
               if(password){
                //Если TRUE пускаем юзера и генерируем ключ
